...
export function ReplayTimeline({ controller, sensorModel }: { controller: ReplayController; sensorModel?: SensorModel }) {
  ...
  return <section className="replay-timeline" aria-labelledby="replay-timeline-heading">
    ...
    <div className="replay-track-filters" role="group" aria-label="Timeline tracks">
      {REPLAY_TRACKS.map((track) => <label key={track.label}><input type="checkbox" checked={track.kinds.every((kind) => selectedKinds.includes(kind))} disabled={!ready} onChange={(event) => updateTrack(track, event.target.checked)} /> {track.label}</label>)}
    </div>
    <div className="replay-analysis-filters" role="group" aria-label="Evidence filters">
      <label>Sensor<select aria-label="Sensor" value={controller.filters.sensorIds[0] ?? ""} disabled={!ready} onChange={(event) => controller.updateFilters({ sensorIds: event.target.value ? [event.target.value] : [] })}><option value="">All sensors</option>{sensorOptions.map((sensor) => <option key={sensor} value={sensor}>{sensor}</option>)}</select></label>
      <label>Resident<select aria-label="Resident" value={controller.filters.actorIds[0] ?? ""} disabled={!ready || !oracle} aria-describedby={!oracle ? "resident-filter-help" : undefined} onChange={(event) => controller.updateFilters(event.target.value ? { actorIds: [event.target.value], selectedResidentId: event.target.value } : { actorIds: [], selectedResidentId: undefined })}><option value="">All residents</option>{residentOptions.map((resident) => <option key={resident} value={resident}>{resident}</option>)}</select></label>
      ...
      <label>Status<select aria-label="Event status" value={controller.filters.statuses[0] ?? ""} disabled={!ready} onChange={(event) => controller.updateFilters({ statuses: event.target.value ? [event.target.value] : [] })}><option value="">All statuses</option>{statusOptions.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
      <label>Zoom<select aria-label="Temporal zoom" value={controller.windowSpanMs} disabled={!ready} onChange={(event) => controller.setWindowSpan(Number(event.target.value))}>{ZOOM_OPTIONS.map(([span, label]) => <option key={span} value={span}>{label}</option>)}</select></label>
      <button type="button" disabled={!ready} onClick={() => controller.updateFilters({ eventKinds: [], sensorIds: [], actorIds: [], selectedResidentId: undefined, statuses: [] })}>Clear filters</button>
    </div>
