...
function HomePage() {
  ...
  const [tab, setTab] = useState<"overview" | "plan" | "runs">("overview");
  // The plan page draws the house and the sensor field together and edits one of them. They
  // used to be two tabs, which meant placing a detector against furniture you could not see
  ...
  return (
    <div className="page home-page">
      ...
      <div className="tabs" role="tablist" aria-label="Home sections">
        {(["overview", "plan", "runs"] as const).map((item) => <button key={item} role="tab" aria-selected={tab === item} onClick={() => setTab(item)}>{item === "plan" ? "Plan & sensors" : item}</button>)}
      </div>
...
function PlanTools({
  ...
  const tools: Array<{ id: PlanTool; label: string; icon: ReactNode; hint: string }> = layer === "home"
    ? [
...
function EditorFields({ layer, selectedId, home, sensors, onHome, onSensors }: { layer: "home" | "sensors"; selectedId: string; home?: HomeModel; sensors?: SensorModel; onHome: (model: HomeModel) => void; onSensors: (model: SensorModel) => void }) {
  ...
  if (layer === "sensors" && sensor && sensors) {
...
function RunPage() {
  ...
  const visibleObservations: Observation[] | undefined = oracle
    ? observations.data?.items
    : observations.data?.items?.map((record) => ({ ...record, oracleCause: undefined }));
  const changeVisibility = (nextOracle: boolean) => {
