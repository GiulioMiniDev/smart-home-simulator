...
def test_authoring_import_is_atomic_and_creates_resident_revisions(tmp_path: Path) -> None:
    ...
    result = service.import_authoring(
        home.home_id,
        payload["scenario"],  # type: ignore[arg-type]
        payload["personalProcessPackage"],  # type: ignore[arg-type]
    )
    ...
    invalid = copy.deepcopy(payload["scenario"])
    invalid["residents"] = []  # type: ignore[index]
    rejected = service.import_authoring(
        home.home_id,
