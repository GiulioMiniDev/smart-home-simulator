# Prompt Semplificato per LLM Locali (v1.2.1-simplified)
...
interface Provenance {
  ...
  generatedAt: string; // Timestamp ISO reale della generazione: non copiarlo dalle date simulate e non inventare una data futura
}
...
interface Activity {
  ...
  allowBoundaryTruncation?: boolean; // true SOLO per attività come il sonno finale che attraversano la fine della simulationWindow
...
1. Per **OGNI** attività inclusa in qualsiasi `DayPlan` dello scenario (`days[].activities`), la combinazione `(actorId, intent)` deve risolvere a **ESATTAMENTE UN** `ProcessBinding` nell'array `personalProcessPackage.bindings`.
2. Se nello scenario compaiono 10 intent distinti per lo stesso residente, devono esistere 10 binding distinti che li coprono tutti. Più binding POSSONO riutilizzare lo stesso `ProcessModel` soltanto quando `implementedComponents` e l'intero flusso di azioni sono realmente identici; non duplicare inutilmente modelli equivalenti.
...
## 8. REGOLE DI PLAUSIBILITÀ E PROVENANCE

La validità dello schema non basta: il piano deve essere umanamente plausibile e fedele alla descrizione del caso.

1. **Sonno e confini temporali**:
   - Per un adulto senza indicazioni diverse, usa un sonno notturno preferito di circa 6-9 ore, non pochi minuti.
   - Il sonno può attraversare la mezzanotte. Per il sonno dell'ultimo giorno mantieni una durata realistica e usa `allowBoundaryTruncation: true` se oltrepassa la fine della `simulationWindow`.
   - Mantieni un intervallo plausibile fra sonno e risveglio del giorno successivo.
2. **Farmaci e condizioni personali**:
   - Genera `take_morning_medication` soltanto se la descrizione della persona menziona una terapia o un bisogno farmacologico.
   - Se generi attività farmacologiche, conserva nel profilo almeno la condizione o la terapia che le motiva. Non inventare farmaci per una persona priva di indicazioni sanitarie.
3. **Coerenza fra nome, orario e luogo**:
   - Intent con `morning` devono avvenire al mattino; intent con `evening` devono avvenire nel tardo pomeriggio o la sera.
   - Una passeggiata deve usare una location coerente con una passeggiata; non usare un bar come destinazione implicita se l'intent non include una visita al bar.
   - `wash_breakfast_dishes` deve seguire la colazione, non il pranzo. Scegli un altro intent se si lavano le stoviglie di un altro pasto.
4. **Completezza e varietà della routine**:
   - Includi pasti, igiene, sonno e impegni coerenti con età, occupazione, giorno della settimana e descrizione fornita.
   - Evita di copiare la stessa giornata cambiando una sola attività: introduci variazioni plausibili senza inventare fatti personali non forniti.
5. **Provenance veritiera**:
   - `generatedAt` rappresenta l'istante reale di generazione, non l'inizio dello scenario e non una data scelta per coerenza narrativa.
   - `modelName` identifica modello, dimensione e quantizzazione realmente usati.
   - Non dichiarare `humanReviewed: true` e non inventare parametri di inference non conosciuti.

---

## 9. CHECKLIST DI AUTO-CORREZIONE PRIMA DI RESTITUIRE IL JSON
...
- [ ] **Provenance**: `generatedAt` è l'istante reale di generazione e non una data futura copiata dallo scenario; `modelName` è quello realmente usato.
...
- [ ] **Copertura Bindings**:
  - Ogni coppia `(actorId, intent)` usata in `days[].activities` risolve a esattamente un binding. I process model identici possono essere riutilizzati.
- [ ] **Plausibilità**:
  - Il sonno ha durata realistica; soltanto il sonno finale può usare `allowBoundaryTruncation: true`.
  - Farmaci e condizioni sanitarie sono supportati dalla descrizione e coerenti con il profilo.
  - Nomi temporali come `morning` ed `evening`, orari, luoghi e ordine dei pasti sono coerenti.
  - I giorni non sono copie identiche salvo che la descrizione richieda esplicitamente una routine invariabile.
