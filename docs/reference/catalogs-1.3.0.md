...
| Fonte | Contenuto |
|---|---|
| `activity-catalog-1.3.0.json` | 93 intenti, 54 componenti |
| `action-catalog-1.1.0.json` | 27 azioni atomiche |
| `variable-catalog-1.0.0.json` | 18 variabili |
| `reference-process-models-1.3.0.json` | 27 modelli di processo |
| `hybrid_planning/intents.py` | alfabeto ridotto di 27 intenti |

> **Versione 1.3.0.** Il catalogo non sapeva rappresentare chi lavora da casa. L'unico intento di
> lavoro era `work_shift`, che la categoria `work` colloca fuori casa e che il preflight obbliga a
> uscire e rientrare dalla porta: una freelance che lavora al tavolo del soggiorno poteva essere
> descritta solo come un'assenza di otto ore che non fa, oppure — ed è quello che gli autori hanno
> fatto — non essere descritta affatto. Un orizzonte generato annota il motivo nei propri assunti
> ("freelance work itself is not declared because no canonical home-work intent exists") e lascia
> la fascia 09:30-17:30 con dentro il solo pranzo, su 260 giorni feriali. La 1.3.0 aggiunge
> `work_from_home`, categoria `home_work`, stanza `living_room`, con il proprio modello di
> riferimento: è un intento domestico come gli altri, e la giornata si dichiara a blocchi
> (`period: day` con `timesPerPeriod`) invece che come un turno unico. Le versioni precedenti
> restano intatte, quindi ogni artefatto già prodotto continua a validare e a riprodursi identico.

| 1.2.0 | 1.3.0 |
|---|---|
| *(nessun intento domestico di lavoro)* | `work_from_home` |

---

## 1. Alfabeto ridotto — 27 intenti
...
## 2. Catalogo completo — 93 intenti
