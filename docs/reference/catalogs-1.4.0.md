...
| Fonte | Contenuto |
|---|---|
| `activity-catalog-1.4.0.json` | 94 intenti, 55 componenti |
| `action-catalog-1.1.0.json` | 27 azioni atomiche |
| `variable-catalog-1.0.0.json` | 18 variabili |
| `reference-process-models-1.4.0.json` | 28 modelli di processo |
| `hybrid_planning/intents.py` | alfabeto ridotto di 28 intenti |

> **Versione 1.4.0.** Il catalogo non sapeva rappresentare l'alzata notturna. Il modello dei drive
> emetteva già le visite in bagno dentro la notte — sono la nocturia, e su un anno generato sono
> decine — ma non esisteva un intento per dirle, così prendevano in prestito
> `morning_toilet_and_wash`, l'unico intento di toilette eseguibile quando quel livello è stato
> scritto. L'intento **è** l'etichetta di ground truth che il dataset pubblica, quindi la verità di
> riferimento diceva "routine del mattino" alle due di notte: 73 occorrenze su 75 fra mezzanotte e
> le sei, su un anno.
>
> L'etichetta era il danno minore. Il processo preso in prestito è un processo diurno e finisce dove
> l'ha lasciata l'ultima azione — in piedi al lavandino — e niente la riportava a letto: `wake_up`
> cominciava camminando dal bagno alla camera su ogni notte in cui i drive le avevano dato una
> visita. Un giorno viene compilato come bundle a sé, quindi non c'è nessun `sleep` in corso da
> riprendere: il rientro deve stare dentro la visita. `night_toilet_visit` dichiara due stanze — il
> bagno in cui accade e la camera in cui finisce — e il suo modello di riferimento si corica. Con lui
> si sposta anche la corrispondenza `Bed_to_Toilet` di CASAS Aruba, che è esattamente questo
> tragitto e non una pausa pomeridiana. Le versioni precedenti restano intatte, quindi ogni
> artefatto già prodotto continua a validare e a riprodursi identico.

| 1.3.0 | 1.4.0 |
|---|---|
| *(nessun intento di alzata notturna)* | `night_toilet_visit` |
| `use_toilet` → `Bed_to_Toilet` | `night_toilet_visit` → `Bed_to_Toilet` |
| *(nessun componente di rientro)* | `return_to_bed` |

---

## 1. Alfabeto ridotto — 28 intenti
...
| `work_from_home` | Work from home | home_work | living_room | `work` | 7 / 6 |
| `use_toilet` | Use the toilet | hygiene | bathroom | `use_toilet` | 7 / 6 |
| `night_toilet_visit` | Night toilet visit | hygiene | bathroom → bedroom | `use_toilet`, `return_to_bed` | 10 / 9 |
| `prepare_and_drink_hot_drink` | Make a hot drink | cooking | kitchen | `prepare_drink`, `consume_drink` | 16 / 15 |
...
## 2. Catalogo completo — 94 intenti
